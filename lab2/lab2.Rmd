...
## Define Function that finds (some-kind of optimal K) 
...
find_best_k <- function(somedata) {
  ...
  for (i in 1:test_len) {
    wss[i] <- kmeans(somedata[, 2:3], centers = i, nstart = 25)$tot.withinss
  }
  
  max_value <- max(wss)
  min_value <- min(wss)
  norm_wss <- (wss - min_value) / (max_value - min_value) * test_len
  plot(1:test_len, norm_wss, type = "b", pch = 16, frame = TRUE, 
       xlab = "Number of Clusters", ylab = "Normalized WSS")
...
ggplot(file_data, aes(x = elec, y = income, color = cluster)) +
  ...
  labs(title = "K-Means Clustering", x = "Electricity Usage", y = "Income")
```
...
ggplot(file_data, aes(x = elec, y = income, color = cluster)) +
  geom_point(size = 3) +
  geom_point(data = centers_df, shape = 8, size = 8) +
  labs(title = "K-Means Clustering (Old K)", x = "Electricity Usage", y = "Income")
...
ggplot(data_log, aes(x = elec, y = income, color = cluster)) +
  geom_point(size = 3) +
  geom_point(data = centers_df, shape = 8, size = 8) +
  labs(title = "K-Means Clustering (New K)", x = "Income", y = "Electricity Usage")
```

## Outlier Removal and Clustering

```{r}
iqr_income <- IQR(data_log$income)
...
outliers <- with(data_log, income < income_thresholds[1] | income > income_thresholds[2] | 
...
ggplot(cleaned_data, aes(x = elec, y = income, color = cluster)) +
  geom_point(size = 3) +
  geom_point(data = centers_df, shape = 8, size = 8) +
  labs(title = "K-Means Clustering (Cleaned Data)", x = "Electricity Usage", y = "Income")
```

## Visualizing Clusters on U.S. Map
